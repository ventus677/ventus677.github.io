...
$actual_role = $stmt_role->fetchColumn();
...
try {
    ...
    // SERVER SIDE ROLE CHECK: Only 'user' gets discount
    $discount_rate = 0.20;
    $expected_discount = 0.00;
    $expected_discount_reason = null;

    if ($actual_role === 'user' && $subtotal_for_verification > 0) {
        $expected_discount = $subtotal_for_verification * $discount_rate;
        $expected_discount_reason = 'Employee discount (20%)';
    }
    ...
    $stmt_insert_order->execute([
        ...
        $discount_amount_to_use, 
        $discount_reason_to_use  
    ]);
    ...
    if ($conn->inTransaction()) { $conn->rollBack(); }
    error_log("General error processing order: " . $e->getMessage());
