...
$allowedSortColumns = [
    'oi.id', 'oi.order_id', 'o.customer_name', 'o.transaction_date',
    'p.product_name', 'oi.quantity', 'oi.price_at_order', 'oi.item_total',
...
if (!empty($searchTerm)) {
    if ($searchColumn === 'oi.quantity' || $searchColumn === 'oi.price_at_order' || $searchColumn === 'oi.item_total' || $searchColumn === 'oi.order_id' || $searchColumn === 'oi.id') {
...
<?php if (!empty($order_items)): ?>
    <table class="order-items-table">
        <thead>
            <tr>
                <th data-column="oi.id">Order Item ID
                    <span class="sort-icon"><?= ($sortColumn === 'oi.id' ? ($sortOrder === 'ASC' ? '▲' : '▼') : '') ?></span>
                </th>
                <th data-column="oi.order_id">Order ID
                    <span class="sort-icon"><?= ($sortColumn === 'oi.order_id' ? ($sortOrder === 'ASC' ? '▲' : '▼') : '') ?></span>
                </th>
                <th data-column="o.customer_name">Customer Name
                    <span class="sort-icon"><?= ($sortColumn === 'o.customer_name' ? ($sortOrder === 'ASC' ? '▲' : '▼') : '') ?></span>
                </th>
                <th data-column="p.product_name">Product Name
                    <span class="sort-icon"><?= ($sortColumn === 'p.product_name' ? ($sortOrder === 'ASC' ? '▲' : '▼') : '') ?></span>
                </th>
                <th data-column="oi.quantity">Quantity Ordered
                    <span class="sort-icon"><?= ($sortColumn === 'oi.quantity' ? ($sortOrder === 'ASC' ? '▲' : '▼') : '') ?></span>
                </th>
                <th data-column="oi.price_at_order">Price at Order
                    <span class="sort-icon"><?= ($sortColumn === 'oi.price_at_order' ? ($sortOrder === 'ASC' ? '▲' : '▼') : '') ?></span>
                </th>
                <th data-column="oi.item_total">Item Total
                ...
                <th data-column="user_name">Input Sale By
        ...
        <tbody>
            ...
            ?>
                <tr>
                    ...
                    <td>PHP <?= htmlspecialchars(number_format($item['price_at_order'], 2)) ?></td>
                    <td>PHP <?= htmlspecialchars(number_format($item['item_total'], 2)) ?></td>
            ...
            <tr class="total-row">
                ...
                <td></td>
                <td>PHP <?= htmlspecialchars(number_format($grand_total_items_value, 2)) ?></td>
                <td></td>
                <td></td>
            </tr>
        </tbody>
